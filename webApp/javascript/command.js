...
async function interrupt () {
  try {
    await fetch(`${get_url()}/8i/interrupt`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function getQueue (clientId) {
  try {
    const res = await fetch(`${get_url()}/8i/queue`)
...
async function getPromptResult (category) {
  let url = get_url()
  try {
    const response = await fetch(`${url}/8i/prompt_result`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function uploadImage (blob, fileType = '.png', filename) {
  ...
  const resp = await fetch(`${url}/8i/upload/image`, {
    method: 'POST',
    body
  })
...
async function uploadMask (arrayBuffer, imgurl) {
  ...
  const resp = await fetch(`${url}/8i/upload/mask`, {
    method: 'POST',
    body
  })
...
async function get_rembg_models () {
  try {
    const response = await fetch(`${get_url()}/8i/folder_paths`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function run_rembg (model, base64) {
  try {
    const response = await fetch(`${get_url()}/8i/rembg`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
